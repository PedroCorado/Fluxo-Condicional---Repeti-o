...
# Aqui eu fiz a interação de cada idade utilizando o for.
# E utilizei o if para saber se a idade e igual ou maior que 30 anos
...
#Estou fazendo a verificação para saber quem clicou no add.
#Quando a pessoa clica em um anuncio , fica com o valor de 1 , e quando nao clica , fica com valor 0
#Na estrutura de comparação eu "pegunto" se tiver clicou_no_ad in dado_de_usuario
#E dado_de_usuario["valor de" clicou_no_ad] nao for vasio e dado_de_usuario[valor] for >= 1
#Ele vai adicionar a renda dos usuarios no array leads.


leads = []

for dado_de_usuario in propaganda_online:
    if 'clicou_no_ad' in dado_de_usuario and dado_de_usuario['clicou_no_ad'] is not None and dado_de_usuario['clicou_no_ad'] >= 1:
        leads.append(dado_de_usuario['renda_area'])

print(leads)


#Aqui eu fiz basicamente a MESMA coisa que o exercicio de cima 


cidades = []

for dado_de_usuario in propaganda_online:
    if 'tempo_gasto_site' in dado_de_usuario and dado_de_usuario['tempo_gasto_site'] is not None and dado_de_usuario['tempo_gasto_site'] >= 70.00:
